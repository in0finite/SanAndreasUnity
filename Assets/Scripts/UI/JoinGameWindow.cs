...
namespace SanAndreasUnity.UI
{
    public class JoinGameWindow : PauseMenuWindow
    {
        ...
        readonly string[] m_tabNames = new string[] { "Internet", "LAN", "Direct" };
...
#pragma warning restore 4014
        ...
        protected override void OnWindowGUI()
        {
            ...
            else if (InternetTabIndex == m_currentTabIndex)
            {
                ...
                foreach (ServerInfo info in _serversFromMasterServer)
                {
                    ...
                    GUILayout.Label(info.Name, GUIUtils.CenteredLabelStyle,
                        GUILayout.Width(availableWidth * widthPercentages[0]));

                    GUILayout.Label($"{info.NumPlayersOnline}/{info.MaxPlayers}", GUIUtils.CenteredLabelStyle,
                        GUILayout.Width(availableWidth * widthPercentages[1]));

                    GUI.enabled = !NetStatus.IsClientActive();
                    if (GUILayout.Button($"{info.IP}:{info.Port}",
                            GUILayout.Width(availableWidth * widthPercentages[2])))
            ...
            else
            {
                ...
                else if (InternetTabIndex == m_currentTabIndex)
                {
                    ...
                    buttonText = _isRefreshingMasterServerList
                        ? ("Refreshing." + new string('.', (int)((Time.time * 2) % 3)))
                        : "Refresh servers";

                    async void ButtonAction() => await RefreshMasterServersButtonPressed();
        ...
        {
            this.Connect(info.EndPoint.Address.ToString(), (ushort)info.EndPoint.Port);
        }
