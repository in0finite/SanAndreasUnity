...
namespace UnityEditor
{
    internal class DesktopPluginImporterExtension : DefaultPluginImporterExtension
    {
        ...
        internal class DesktopSingleCPUProperty : Property
        {
            ...
            internal bool IsTargetEnabled(DLLFileWrapperInspector inspector)
            {
                DLLFileWrapperInspector.Compatibility compatibililty = inspector.GetPlatformCompatibility(platformName);
                if (compatibililty == DLLFileWrapperInspector.Compatibility.Mixed)
                    throw new Exception("Unexpected mixed value for '" + inspector.importer.assetPath + "', platform: " + platformName);
                return compatibililty == DLLFileWrapperInspector.Compatibility.Compatible;
            }

            internal override void OnGUI(DLLFileWrapperInspector inspector)
            {
        ...
        private Property[] GetProperties()
        {
            ...
            m_WindowsX86 = new DesktopSingleCPUProperty(FEditorGUI.TrTextContent("x86"), FEditor.GetBuildTargetName(BuildTarget.StandaloneWindows));
            m_WindowsX86_X64 = new DesktopSingleCPUProperty(FEditorGUI.TrTextContent("x86_x64"), FEditor.GetBuildTargetName(BuildTarget.StandaloneWindows64));

            m_LinuxX86 = new DesktopSingleCPUProperty(FEditorGUI.TrTextContent("x86"), FEditor.GetBuildTargetName(BuildTarget.StandaloneLinux), DesktopPluginCPUArchitecture.x86);
            m_LinuxX86_X64 = new DesktopSingleCPUProperty(FEditorGUI.TrTextContent("x86_x64"), FEditor.GetBuildTargetName(BuildTarget.StandaloneLinux64), DesktopPluginCPUArchitecture.x86_64);

            m_OSX_X64 = new DesktopSingleCPUProperty(FEditorGUI.TrTextContent("x64"), FEditor.GetBuildTargetName(BuildTarget.StandaloneOSX));
        ...
        public override void OnPlatformSettingsGUI(DLLFileWrapperInspector inspector)
        {
            PluginImporter imp = inspector.importer;
            EditorGUI.BeginChangeCheck();
            if (IsUsableOnWindows(imp))
            {
                EditorGUILayout.LabelField(FEditorGUI.TrTextContent("Windows"), EditorStyles.boldLabel);
            ...
            {
                EditorGUILayout.LabelField(FEditorGUI.TrTextContent("Linux"), EditorStyles.boldLabel);
            ...
            {
                EditorGUILayout.LabelField(FEditorGUI.TrTextContent("Mac OS X"), EditorStyles.boldLabel);
        ...
        public void ValidateSingleCPUTargets(DLLFileWrapperInspector inspector)
        {
        ...
        private void ValidateUniversalTargets(DLLFileWrapperInspector inspector)
        {
            ...
            inspector.SetPlatformCompatibility(FEditor.GetBuildTargetName(BuildTarget.StandaloneLinuxUniversal), linuxX86Enabled || linuxX86_X64Enabled);
            ...
            inspector.SetPlatformCompatibility(FEditor.GetBuildTargetName(BuildTarget.StandaloneOSX), osxX64Enabled);
        }

        public override string CalculateFinalPluginPath(string platformName, PluginImporter imp)
        {
            BuildTarget target = FEditor.GetBuildTargetByName(platformName);
